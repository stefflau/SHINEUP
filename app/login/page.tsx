...
export default function AuthPage() {
  ...
  const handleAuth = async () => {
    ...
    if (mode === "signup") {
      ...
      // Vérifier si le numéro WhatsApp est déjà utilisé
      ...
      if (signErr) {
        setError(translateError(signErr.message));
        setLoading(false);
        return;
      }

      // Supabase renvoie un user même si l'email existe déjà (identities vide)
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#0a0a0a", color: "#f0f0f0", display: "flex", alignItems: "center", justifyContent: "center", padding: "1rem" }}>
      <div style={{ width: "100%", maxWidth: "420px", background: "#141414", borderRadius: "20px", padding: "2rem" }}>
        ...
        {/* Champs signup uniquement */}
        {mode === "signup" && (
          <>
            <div style={{ marginBottom: "12px" }}>
              <input
                style={inputStyle}
                placeholder="WhatsApp (ex: 0341234567)"
                type="tel"
                value={whatsapp}
